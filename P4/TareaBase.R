...
grietas <- data.frame()
for (i in c(10, 30, 50, 90, 120, 180)){
 
for (k in c(5, 18, 44, 83, 99)){ 
...
propaga <- function(replica) {
  ...
  #Opcional para ver cómo se forman las grietas en la zona
...
grietas <- rbind(grietas, largos)
summary(largos)
}
}

t.grietas <- t(grietas)
boxplot(t.grietas, col=c(3,4,5,6,7), xlab="Tamaños de 10, 30, 50, 90, 120 y 180", ylab="Largos de grietas")