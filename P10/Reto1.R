...
reproducir <- function(i){
  padres <- sample(1:tam, 2, replace=FALSE, prob = proba)
...
for (iter in 1:tmax) {
  ...
  for(i in 1: length(obj)){
    proba <- c(proba, .9*obj[i] + (1-obj[i])*.1)
  }
  clusterExport(cluster, c("p", "tam", "proba"))
  ...
  mejores.a <- c(mejores.a, mejor)
}
stopCluster(cluster)
png("reto1.png", width=600, height=300)
plot(1:tmax, mejores.a, xlab="Paso", ylab="Mayor valor", type='l', ylim=c(0.95*min(mejores.a), 1.05*optimo))
points(1:tmax, mejores.a, pch=15)
