...
for(k in c(seq(2, 10, 2))){
  ...
  val <- parSapply(cluster, 1:n, evaluaciones)
  val <- t(val)
  ...
  porcentaje <- c(porcentaje, sum(no.dom)/n)
  png(paste("p11_frente_",k, ".png", sep=""))
  plot(val[,1], val[,2], xlab=paste(xl, "mejor con cuadro azul"),
       ylab=paste(yl,"mejor con bolita naranja"),
       main="Ejemplo bidimensional")
...
plot(y=porcentaje, x= seq(2,10,by=2), xlab="Variaciones en k", ylab="Porcentaje obtenido")
