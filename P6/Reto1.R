...
infectados.probv <- data.frame()
pv <- seq(0.05, 0.5, 0.05)
for ( probv in pv){
l <- 1.5
n <- 50
pi <- 0.05
pr <- 0.02
v <- l / 30
agentes <- data.frame(x = runif(n, 0, l), y = runif(n, 0, l), dx = runif(n, -v, v), dy = runif(n, -v, v), estado = sample(c("S", "I", "R"), n, replace=TRUE, c(1-(pi+probv), pi, probv)))
...
for (tiempo in 1:tmax) {
  ...
  final <- data.frame()
  final <- rbind(final ,foreach(i = 1:n, .combine=c) %dopar% actualiza())
  final1 <- data.frame()
...
infectados.probv <- rbind(infectados.probv, max(epidemia))
...
colnames(infectados.probv) <- 1
rownames(infectados.probv) <- seq(0.05, 0.5, 0.05)
t.inf.probv <- t(infectados.probv)

plot(t.inf.probv[1,], xlab="Probabilidades distintas de vacunados", ylab="Infectados")
